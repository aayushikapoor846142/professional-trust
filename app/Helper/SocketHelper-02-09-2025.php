    ...
    function groupAttachments($groupId, $page = 1, $perPage = 50, $search = "")
{
    ...
    $html = view('admin-panel.01-message-system.group-chat.groups.chat.partials.attachments', compact('groupAttachments'))->render();
...
if(!function_exists("chatAttachments")){
    ...
    function chatAttachments($chatId, $page = 1, $perPage = 50, $search = "")
{
    $search = request()->get('search', '');
    $page = request()->get('page', $page);
    $type=request()->get('type','');
...
if(!function_exists("globalNotification")){
    function globalNotification($arr)
    {
        ...
        if($arr['type'] == "post_case"){
         ...
         if($arr['type'] == "new_case_proposal"){
              ChatNotification::create([
                'comment' => $arr['comment'] ?? null,
                'type' => $arr['type'] ?? null,
                'redirect_link' => $arr['redirect_link'] ?? null,
                'is_read' => $arr['is_read'] ?? 0,
                'user_id' => $arr['user_id'] ?? null,
                'send_by' => $arr['send_by'] ?? null,
            ]);
            $notif_data_new = [
                "action" => "new_case_proposal",
                "receiver_id" =>  $arr['user_id'],
                "message"=>$arr['comment'],
                "case_proposals_count"=>$arr['case_proposals_count'],
                "case_unique_id" =>  $arr['case_id'],

            ];
            initGlobalNotification($arr['user_id'], $notif_data_new);
