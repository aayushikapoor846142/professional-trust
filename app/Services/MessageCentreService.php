...
class MessageCentreService
{
        ...
        try {
            ...
            if($request->attachment!=NULL || $message!=NULL){
               ...
               if($chat_message->message || $chat_message->attachment!=NULL){
                ...
                $view = \View::make('admin-panel.01-message-system.individual-chats.sent-message', $viewData);
                ...
                initChatMessageSocket($receiver_id, $socket_data);
    ...
    public function getChatUsers($user_id,$conversation_id=0)
    {
        ...
        $chat_users = Chat::with(['lastMessage', 'addedBy', 'chatWith'])
                        ->where(function ($query) use($user_id){
                            $query->where('user2_id', $user_id)
                                ->orWhere('user1_id', $user_id);
                        })
                        ->addSelect(['last_message_date' => ChatMessage::query()
                            ->select('created_at')
                        ...
                        ->get();
        // if($chat){
        //     $chat_users = $chat_users->sortByDesc(function($chat) {
    ...
    public function fetchIndividualChats($chat_id, $last_msg_id, Request $request)
    {
        try {
            $chat = Chat::where('unique_id', $chat_id)->with(['chatWith','addedBy'])->first();
            if(empty($chat)){
                return [
                    'status' => false,
                    'data' => [],
            ...
            $last_sent_message = ChatMessage::withTrashed()->with('sentBy')
                        ...
                        ->latest()
                        ->first();
            $last_read_message = ChatMessageRead::where("chat_id",$chat->id)
                                ->where("receiver_id",$receiver_id)
                                ->where('status','read')
                                ->orderBy("id","desc")
                                ->first();
            ...
            if(!empty($msg_ids)){
            ChatMessageRead::where('chat_id',$chat->id)
                    ->where("receiver_id",auth()->user()->id)
                    ->update(['status'=>'read']);
            ...
            // Prepare response data in the expected format
            $return_data = [];
            $return_data['status'] = true;
            $return_data['data'] = [];
            $return_data['data']['first_message_id'] = $first_msg_id;
            $return_data['data']['last_msg_read'] = false;
            // $return_data['data']['last_msg_unique_id'] = $last_message?$last_message->unique_id:0;
            $return_data['data']['last_message_id'] = $last_message?$last_message->id:0;
            $return_data['data']['message_id'] = $last_message?$last_message->unique_id:0;
            $last_read_message_id =  $last_read_message->message_id??0;
            $return_data['data']['last_read_message'] = $last_read_message_id;
            $return_data['data']['receiver_id'] = $receiver_id;
            $return_data['data']['sender_id'] = auth()->user()->id;
            $return_data['data']['chat_messages'] = $chat_messages;
            $return_data['data']['chat'] = $chat;
            $return_data['data']['unread_count'] = unreadTotalChatMessages($userId);
            $return_data['data']['total_unread'] = unreadTotalChatMessages($userId) + unreadTotalGroupMessages($userId);
            $return_data['data']['read_msg_ids'] = $msg_ids;
         
            if($last_sent_message){
                if($last_read_message_id == $last_sent_message->id){
                    $return_data['data']['last_msg_read'] = true;
                }else{
                    $return_data['data']['last_msg_read'] = false;
                }
            }else{
            ...
            if(isset($last_read_message->id)){
                if(($prev_msg_read->message_id??0) == $last_read_message->id){
                    $return_data['data']['prev_msg_read'] = 0;
                }else{
                    $return_data['data']['prev_msg_read'] = $prev_msg_read->message_id??0;
                }
            }else{
                $return_data['data']['prev_msg_read'] = 0;
            }
            ...
            if($chat->blocked_chat==1){
                $return_data['data']['is_blocked'] = true;
            }else{
                $return_data['data']['is_blocked'] = false;
            }
            $return_data['data']['is_typing'] = $is_typing;
            userActiveStatus();    
            updateMessagingBox(auth()->user()->id,$receiver_id);
    ...
    {
        $chat = Chat::where('unique_id', $chatId)->first();
        $userId = auth()->user()->id;
        $attachments = ChatMessage::where('chat_id', $chat->id)
