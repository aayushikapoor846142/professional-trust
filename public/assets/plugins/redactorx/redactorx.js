    ...
    encodeAttrSings: function(html) {
        ...
        if (matches !== null) {
            for (var i = 0; i < matches.length; i++) {
                ...
                var str = matches[i].replace(/>/g, 'xmoresignz');
                str = str.replace(/</g, 'xlesssignz');
...
RedactorX.add('module', 'inline', {
    ...
    _formatUncollapsed() {
        let $nodes = false;
        let selectedAll = this.app.editor.isAllSelected();
        let $blocks = this.dom(this.app.selection.getNodes({ type: 'blocks', partial: true }));
        let inlines = this.app.selection.getNodes({ type: 'inline' });
...
RedactorX.add('module', 'parser', {
    ...
    _unparseEmbed: function($node) {
        ...
        var $responsive = $node.find('.' + this.opts.embed.responsive.replace(/ +/g, '.'));
...
RedactorX.add('module', 'embed', {
    ...
    _parseUrl: function(str) {
        ...
        if (str.match(this.opts.regex.youtube)) {
            ...
            parsed = str.replace(this.opts.regex.vimeo, 'https://player.vimeo.com/video/$2');
            return iframeStart + parsed + iframeEnd;
        }
...
RedactorX.add('module', 'context', {
    ...
    _buildPosition: function(e) {
        ...
        var pos = this.app.selection.getPosition();
        var posEnd = this.app.selection.getPosition('end');
        var pageX = (e) ? e.pageX : posEnd.left;
        var left = (pageX - width/2);
        var top = pos.bottom + this.app.$doc.scrollTop();
        var topFix = 2;
        var leftFix = 4;
...
RedactorX.add('block', 'block.list', {
    ...
    handleEnter: function(e) {
        e.preventDefault();
        var $newItem, $currentItem, currentItem, isItemEmpty, $prevItem;
        ...
        // middle
            ...
            if (isItemEmpty) {
                ...
                // split list
                $prevItem = $newItem.prevElement();
                if (this.app.content.isEmptyHtml($prevItem.html())) {
                    e.preventDefault();
                    var $part = this.app.element.split(this.$block);
                    var text = this.app.block.create();
                    this.insert({ instance: text, caret: 'start'});
