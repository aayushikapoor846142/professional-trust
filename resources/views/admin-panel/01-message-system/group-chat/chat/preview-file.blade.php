    ...
    <div class="modal-content">
        <div class="modal-header d-flex justify-content-between">
            ...
            <div class="d-flex justify-content-between align-items-center gap-3">
                <a href="{{ url('download-media-file?dir='.groupChatDir().'&file_name='.$file_name) }}"
                    class="popup-link" data-download="{{ groupChatDirUrl($file_name, 'r') }}">
                    <i class="fa fa-download fa-lg white"></i>
                </a>
                @if($chatMessage->user_id == auth()->user()->id)
                <a href="javascript:;"
                    onclick="deleteSelectedGroupAttachmentMessage('{{ $chatMessage->unique_id }}', '{{ $file_name }}')">
        ...
        <div class="modal-body">
            ...
            // Image File Preview
            $previewHtml = '<div class="w-100 mx-auto text-center"><img src="' . htmlspecialchars($fileUrl) . '"
                    alt="Preview" style="max-width: 100%; max-height: 500px;" /></div>';
            ...
            $previewHtml = '<div class="text-center">Preview not available.';
                $previewHtml .= '<a href="'.url('download-media-file?dir='.groupChatDir().'&file_name='.$file_name).'"
                    class="img-fluid" data-download="'.groupChatDirUrl($file_name, 'r').'">
                    Click to download
                </a></div>';
            }
            @endphp
