            ...
            <div class="chat-profile-header text-center p-4 group-info group-sidebar-heading" id="group-name-edit">
                ...
                <div class="cds-rightGrpInfo">
                    <div class="group-info-actionbtn">
                        ...
                        <a href="javascript:;" onclick="confirmAction(this)" data-href="{{ baseUrl('group-message/delete-group-message/'.$group->unique_id) }}">
                    ...
                    @if($currentGroupMember->is_admin==1)
                        <a class="message-upload-file" href="javascript:;" onclick="editGroup('{{$group->unique_id}}')">
                        ...
                        <div class="accordion-body p-0">
                            ...
                            <div class="w-100 @if($member->deleted_at){{'chat-disabled'}}@endif " for="member-list-{{$member->member->id}}">
                                <div class="chat-item chat-request group-item">
                                    ...
                                    @if($currentGroupMember->is_admin==1)
                                    <a href="javascript:;" onclick="confirmAction(this)" data-href="{{ baseUrl('group-message/remove-group-member/'.$member->id) }}">
                                        <i class="fa-regular fa-trash text-danger"></i>
                                    </a>

                                    @if($member->is_admin!=1)
                                    <a data-href="{{ baseUrl('group-message/make-group-admin/'.$member->unique_id) }}" onclick="confirmAnyAction(this)" title="Mark as Admin" data-action="Make Group Admin">
                                        <i class="fa-regular fa-user text-primary"></i>
                                    </a>
                                    @elseif($member->is_admin == 1)
                                    <a data-href="{{ baseUrl('group-message/remove-group-admin/'.$member->unique_id) }}" onclick="confirmAnyAction(this)" title="Remove as Admin" data-action="remove from Group Admin">
                            ...
                            <div class="w-100">
                                <div class="chat-item cds-leave-group">
                                    <a href="javascript:;" title="Leave Group" data-action="leave this Group" onclick="checkRemoveadmin(this)" data-href="{{ baseUrl('group-message/remove-group-member/'.$member->id) }}">
                                        Leave Group
...
<script>
    function editGroup(unique_id){
        closeRightSlidePanel();
        showPopup('<?php echo baseUrl('group-message/edit-group/' . $group->unique_id) ?>');
    }
</script>