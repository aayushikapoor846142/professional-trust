...
<div class="group-messages-message sent" data-message-id="{{ $group_message->unique_id }}" id="message-{{ $group_message->unique_id }}">
    <div class="group-messages-message-content">
        <div class="group-messages-message-bubble">
            ...
            @if($group_message->reply_to != NULL && !empty($group_message->replyTo))
                <div class="group-messages-reply-to" onclick="scrollToGrpMessage(this,'{{ $group_message->group_id }}','{{ $group_message->replyTo->unique_id }}')">
                    @if($group_message->replyTo->message != NULL)
                        <span class="group-messages-reply-username">{!! makeBoldBetweenAsterisks($group_message->replyTo->message) !!}</span>
                    @endif
            ...
            <div class="group-messages-message-text">
                @if($group_message->edited_at)
                    <span class="group-messages-edited-indicator" id="editedMsg{{ $group_message->unique_id }}">edited</span>
                @endif
            ...
            @if($group_message->attachment && $group_message->deleted_at == NULL)
                <div class="group-messages-message-attachment {{ (count(explode(',', $group_message->attachment)) > 1) ? 'files-uploaded' : '' }}">
            ...
            @if($group_message->messageReactions && $group_message->messageReactions->count() > 0)
                <div class="group-messages-message-reactions">
                    ...
                    <div class="group-messages-reactions-tooltip" id="reactionsList{{ $group_message->unique_id }}">
                        ...
                        <span class="group-messages-reactions-count">{{ $group_message->messageReactions->count() }}</span>
                            ...
                            <div class="group-messages-reactions-users">
                                @foreach($group_message->messageReactions as $msgReaction)
                                    <div class="group-messages-reaction-user">
                                        <div class="group-messages-reaction-user-avatar" data-initial="{{ userInitial($msgReaction->userAdded) }}">
            ...
            @if($group_message->deleted_at == NULL)
                <div class="group-messages-message-actions">
                    <div class="group-messages-message-options">
                        <button class="group-messages-options-btn" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                            <i class="fa-solid fa-ellipsis"></i>
                        </button>
                        <ul class="group-messages-dropdown-menu">
                            @if($group_message->message != NULL)
                                <li>
                                    <a class="group-messages-dropdown-item" href="javascript:;" onclick="groupCopyMessage('cpMsg{{ $group_message->unique_id }}')">
                                        Copy <i class="fa fa-copy"></i>
                                    </a>
                                </li>
                                <li>
                                    <a class="group-messages-dropdown-item" href="javascript:;" data-message-id="{{ $group_message->unique_id }}" onclick="editGroupMessage('{{ $group_message->unique_id }}')">
                                        Edit <i class="fa fa-edit"></i>
                                    </a>
                                </li>
                            @endif
                            <li>
                                <a class="group-messages-dropdown-item replyTo" href="javascript:;" onclick="groupReplyTo(this,'{{ $group_message->group_id }}','{{ $group_message->id }}')">
                                    Reply <i class="fa fa-reply"></i>
                                </a>
                            </li>
                            <li>
                                <a class="group-messages-dropdown-item" href="javascript:;" data-message-id="{{ $group_message->unique_id }}" onclick="deleteGroupMessage(this)">
                                    Delete for me <i class="fa fa-trash"></i>
                                </a>
                            </li>
                            <li>
                                <a class="group-messages-dropdown-item" href="javascript:;" data-message-id="{{ $group_message->unique_id }}" onclick="deleteGroupMessageForAll(this)">
                                    Delete for all <i class="fa fa-trash"></i>
                                </a>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="group-messages-emoji-reaction-area">
                        <div class="group-messages-reaction-input">
                            <div class="group-messages-emoji-icon message-reaction" data-message-id="{{ $group_message->unique_id }}" id="emojiBtn-{{ $group_message->unique_id }}">
