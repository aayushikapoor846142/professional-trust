@foreach(explode(',',$get_attachments) as $attachment)
    ...
    @if(\Illuminate\Support\Str::endsWith($attachmentLower, ['.jpg', '.jpeg', '.png', '.gif', '.webp']))
        <div class="attachment" data-file-name="{{ $attachment }}">
            <a href="javascript:;" onclick="showPopup('<?php echo baseUrl('group/preview-file?file_name='.$attachment.'&chat_id='.$chat_msg_Unique_id) ?>')" class="popup-link" title="{{$attachment}}">
        ...
        <div class="attachment" data-file-name="{{ $attachment }}">
            <a href="javascript:;" onclick="showPopup('<?php echo baseUrl('group/preview-file?file_name='.$attachment.'&chat_id='.$chat_msg_Unique_id) ?>')" class="popup-link cds-pdfPreview" title="{{$attachment}}">
                @php
                $pdfid = "pdf-".mt_rand();
