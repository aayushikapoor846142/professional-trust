<div class="CdsIndividualChat-message-wrapper received" id="message-{{ $message->unique_id }}"   data-message-timestamp="{{ $message->created_at->toISOString() }}">
    <!-- Clear Chat Checkbox -->
    ...
    <div class="CdsIndividualChat-message received">
        <!-- Message Reactions -->
        ...
        @if($message->messageReactions && count($message->messageReactions) > 0)
            ...
            <span class="CdsIndividualChat-reaction" onclick="removeIndividualChatReaction(this)" data-message-id="{{ $reaction->unique_id }}" id="MsgReaction{{$reaction->unique_id}}">
                {{$reaction->reaction}}
            </span>
            @endforeach
        @endif
