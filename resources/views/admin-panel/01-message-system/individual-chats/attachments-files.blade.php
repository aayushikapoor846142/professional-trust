...
@foreach(explode(',',$file['attachment']) as $attachment)
@php
...
<div class="attachment" data-file-name="{{ $attachment }}">
    <a href="javascript:;"
        onclick="showPopup('<?php echo baseUrl('individual-chats/preview-file?file_name='.$attachment.'&chat_id='.$chat_msg_Unique_id) ?>')"
        class="popup-link" title="{{$attachment}}">
        <img src="{{ chatDirUrl($attachment, 's') }}" class="img-fluid msg-image-show" alt="Images" />
    </a>
</div>
...
<div class="attachment" data-file-name="{{ $attachment }}">
    <a href="javascript:;"
        onclick="showPopup('<?php echo baseUrl('individual-chats/preview-file?file_name='.$attachment.'&chat_id='.$chat_msg_Unique_id) ?>')"
        class="popup-link cds-pdfPreview" title="{{$attachment}}">
        @php
        $pdfid = "pdf-".mt_rand();
        $fileurl = chatDirUrl($attachment, 'r');
        $thumbimg = chatDirUrl($attachment, 'r',true);
        @endphp
        <div class="pdfView">
            <img src="{{$thumbimg}}" alt="Thumbnail Preview" class="img-fluid pdf-thumbnail">
        </div>
        <div class="bot-fileTitle">
            {{--<img src="{{url('assets/images/chat-icons/pdf-icon.png')}}" alt="File Icon" class="img-fluid pdf-icon"
            />--}}
    ...
    <div class="attachment-docs">
        <a href="javascript:;"
            ...
            <div class="cds-smalIcons song-title">
                <img src="{{url('assets/images/chat-icons/headphone.svg')}}" alt="File Icon"
                    class="file-mp3-img img-fluid" />
                <p class="file-name">{{$attachment}}</p>
            </div>
        </a>
    </div>
    ...
    <div class="attachment-docs">
        <a href="javascript:;"
            ...
            <div class="cds-smalIcons video-title">
                <img src="{{url('assets/images/chat-icons/video.svg')}}" alt="File Icon"
                    class="file-video-img img-fluid" />
                <p class="file-name">{{$attachment}}</p>
            </div>
        </a>
    </div>
...
@elseif(\Illuminate\Support\Str::endsWith($attachmentLower, ['.dpc']))
<div class="attachment" data-file-name="{{ $attachment }}">
    <a href="javascript:;"
        onclick="showPopup('<?php echo baseUrl('individual-chats/preview-file?file_name='.$attachment.'&chat_id='.$chat_msg_Unique_id) ?>')"
        class="popup-link" title="{{$attachment}}">
...
<div class="attachment" data-file-name="{{ $attachment }}">
    ...
    @endphp
    <a href="javascript:;"
        onclick="showPopup('<?php echo baseUrl('individual-chats/preview-file?file_name='.$attachment) ?>')"
        class="popup-link" title="{{$attachment}}">
        <div class="cds-smalIcons">
