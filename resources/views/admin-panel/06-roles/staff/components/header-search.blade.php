<div class="CdsTYDashboard-special-filters-page-container">
                ...
                <div class="CdsTYDashboard-special-filters-dropdown-container">
                    <div class="CdsTYDashboard-special-filters-buttons-row">
                        ...
                        <button class="CdsTYDashboard-special-filters-dropdown-btn" id="budgetBtn" onclick="cdsfilterTogglePanel('budget')">
                            <span>Roles <span class="CdsTYDashboard-special-filters-badge" id="budgetCount" style="display: none;">0</span></span>
                            <span class="CdsTYDashboard-special-filters-arrow">▼</span>
                        </button>
                    </div>
                    ...
                    <div class="CdsTYDashboard-special-filters-panel-wrapper" id="budgetPanel">
                        <div class="CdsTYDashboard-special-filters-panel">
                            <div class="CdsTYDashboard-special-filters-panel-header">
                                <h3 class="CdsTYDashboard-special-filters-panel-title">Roles</h3>
                                <button class="CdsTYDashboard-special-filters-close-panel" onclick="cdsfilterClosePanel('budget')">✕</button>
                            </div>
                            <div class="CdsTYDashboard-special-filters-panel-body">
                                <div class="CdsTYDashboard-special-filters-section">
                                    <div class="CdsTYDashboard-special-filters-section-title">Roles</div>
                                    @if(!empty($staffRoles))
                                        @foreach($staffRoles as $value)
                                            <div class="CdsTYDashboard-special-filters-checkbox-item">
                                                <input type="checkbox" id="status-{{$value}}" value="{{$value}}" class="CdsTYDashboard-role-filter" data-category="service">
                                                <label for="status-{{$value}}" class="CdsTYDashboard-special-filters-checkbox-label">
                                                    {{ucfirst($value)}}
                                                    <!-- <span class="CdsTYDashboard-special-filters-count">(353)</span> -->
                                                </label>
 ...
 <script>
        ...
        // Helper function to find and call loadData
        function callLoadData() {
            console.log('Attempting to call loadData...');
            if (typeof loadData === 'function') {
                console.log('loadData found in local scope');
                return loadData();
            } else if (typeof window.loadData === 'function') {
                console.log('loadData found in window scope');
                return window.loadData();
            } else if (typeof parent.loadData === 'function') {
                console.log('loadData found in parent scope');
                return parent.loadData();
            } else {
                console.error('loadData function not found in any scope');
                // Try to trigger a custom event that the parent can listen to
                const searchEvent = new CustomEvent('staffSearchChanged', {
                    detail: { searchTerm: document.getElementById('searchInput')?.value || '' }
                });
        ...
        document.addEventListener('DOMContentLoaded', () => {
            console.log('Header search component loaded');
            console.log('Available loadData functions:');
            console.log('- Local scope:', typeof loadData);
            console.log('- Window scope:', typeof window.loadData);
            console.log('- Parent scope:', typeof parent?.loadData);
