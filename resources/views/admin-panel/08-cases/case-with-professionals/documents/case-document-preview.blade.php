    ...
    <div class="CdsDocumentPreview-content">
        ...
        <div class="CdsDocumentPreview-previewWrapper">
            <div class="CdsDocumentPreview-previewContainer">
                ...
                <button class="CdsDocumentPreview-commentsBtn" onclick="cdsDocumentToggleComments()">
                ...
                <div class="CdsDocumentPreview-commentsPanel" id="cdsDocumentCommentsPanel">
                    <div class="CdsDocumentPreview-commentsHeader">
                        <h3 class="CdsDocumentPreview-commentsTitle">Comments</h3>
                        <button class="CdsDocumentPreview-commentsClose" onclick="cdsDocumentToggleComments()">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    ...
                    <div class="CdsDocumentPreview-commentsBody" id="cdsDocumentCommentsBody">
                        <!-- Comments will be loaded here -->
                    </div>
                    <div class="CdsDocumentPreview-commentForm">
                        <form onsubmit="cdsDocumentAddComment(event)" id="cdsDocumentCommentForm">
                            <input type="hidden" name="parent_id" id="cdsDocumentPreview-parent-id" value="0"/>
                            <div class="CdsDocumentPreview-comment-reply"></div>
                            <div class="CdsDocumentPreview-commentInputWrapper">
                                <textarea class="CdsDocumentPreview-commentTextarea" id="cdsDocumentCommentText" placeholder="Add a comment..."></textarea>
                                <div class="CdsDocumentPreview-attachmentPreview" id="cdsDocumentAttachmentPreview" style="display: none;">
                                    <div class="CdsDocumentPreview-attachmentItem">
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                            <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                                            <polyline points="14 2 14 8 20 8"/>
                                        </svg>
                                        <span class="CdsDocumentPreview-attachmentName"></span>
                                        <button type="button" class="CdsDocumentPreview-attachmentRemove" onclick="cdsDocumentRemoveAttachment()">
                            ...
                            <div class="CdsDocumentPreview-commentActions">
                                <input type="file" id="cdsDocumentAttachmentInput" style="display: none;" onchange="cdsDocumentHandleAttachment(event)">
                                <button type="button" class="CdsDocumentPreview-attachBtn" onclick="document.getElementById('cdsDocumentAttachmentInput').click()">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/>
                                    </svg>
                                    Attach
                                </button>
                                <button type="submit" class="CdsDocumentPreview-commentSubmit">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor">
...
<script>
    ...
    function cdsDocumentClosePreview() {
        ...
        $("#cdsCaseDocumentPreviewOverlay").html('');
        $("#cdsCaseDocumentPreviewOverlay").removeClass("CdsCaseDocumentPreview-active");
    ...
    // Toggle comments
    function cdsDocumentToggleComments() {
        var panel = document.getElementById('cdsDocumentCommentsPanel');
        panel.classList.toggle('CdsDocumentPreview-show');
        //   var file = cdsDocumentFiles[0];
    ...
    // Load comments with enhanced UI
    function cdsDocumentLoadComments(document){
        $.ajax({
            url: "{{ baseUrl('case-with-professionals/fetch-document-comments') }}",
            type: "post",
            data: {
                _token:"{{ csrf_token() }}",
            ...
            success: function(response) {
                ...
                if (response.status == true) {
                    $("#cdsDocumentCommentsBody").html(response.contents);
                }
            },
            error: function() {
                internalError();
    ...
    function replyComment(e) {
        ...
        $("#cdsDocumentPreview-parent-id").val(comment_id);
        var commentText = $("#case-document-comment-"+comment_id).find(".CdsDocumentPreview-commentArea").html();
        $("#cdsDocumentCommentForm").find(".CdsDocumentPreview-comment-reply").html(commentText)
    }
    // Enhanced add comment function
    function cdsDocumentAddComment(event) {
        event.preventDefault();
        
        var textarea = document.getElementById('cdsDocumentCommentText');
        ...
        // Handle attachment
        if (cdsDocumentCurrentAttachment) {
            newComment.attachment = {
                name: cdsDocumentCurrentAttachment.name,
                size: cdsDocumentFormatFileSize(cdsDocumentCurrentAttachment.size),
                url: '#' // This would be the uploaded file URL
            };
        }
        
        // Add to current file
        if (!cdsDocumentFiles[cdsDocumentCurrentIndex].comments) {
            cdsDocumentFiles[cdsDocumentCurrentIndex].comments = [];
        }
        cdsDocumentFiles[cdsDocumentCurrentIndex].comments.unshift(newComment);
        
        // Save comment to database
        cdsDocumentSaveComment(cdsDocumentFiles[cdsDocumentCurrentIndex], text, cdsDocumentCurrentAttachment);
        
        // Reload comments
        cdsDocumentLoadComments(cdsDocumentFiles[cdsDocumentCurrentIndex]);
        
        // Clear form
        textarea.value = '';
        cdsDocumentRemoveAttachment();
        
        // Show toast
        cdsDocumentShowToast('Comment added successfully!');
    }

    // Save comment to database
    function cdsDocumentSaveComment(document, commentText, attachment = null) {
        ...
        formData.append('parent_id',$("#cdsDocumentPreview-parent-id").val());
        if (attachment) {
            formData.append('attachment', attachment);
        }
        
       $.ajax({
            url: "{{ baseUrl('case-with-professionals/save-document-comment') }}",
            type: "post",
            data: formData,
            cache: false,
            contentType: false,
            ...
            success: function(response) {
                ...
                if (response.status == true) {
                    ...
                    $("#cdsDocumentPreview-parent-id").val('');
                    $("#cdsDocumentCommentForm").find(".CdsDocumentPreview-comment-reply").html('')
                } else {
