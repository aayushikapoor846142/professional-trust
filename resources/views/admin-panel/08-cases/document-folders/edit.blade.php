...
<div class="cds-form-container cds-ty-dashboard-box-body mb-0">
    ...
    <form id="form" class="js-validate mt-3" action="{{ baseUrl('/document-folders/update/'.$record->unique_id) }}" method="post">
        @csrf
        <div class="row">
            <div class="col-xl-12">
                 {!! FormHelper::formInputText(['name'=>"name","id" => "name","value" =>$record->name ?? '',"label"=>"Enter name","required"=>true,'events'=>['oninput=validateString(this)']]) !!}     
            </div>
        </div>        
