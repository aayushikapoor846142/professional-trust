...
@section('content')
 <div class="ch-action">
                    <a href="{{ baseUrl('manage-services/list-assessment/'.$record->unique_id) }}" class="CdsTYButton-btn-primary">
...
<div class="CDSDashboardContainer-container" id="CDSDashboardContainer-dashboardContainer">
 ...
 <form id="popupForm" action="{{ baseUrl('/manage-services/generate-assessment/'.$record->unique_id) }}" method="post" enctype="multipart/form-data" class="question-form">
                            @csrf
                            <div class="row">
                                <div class="col-md-12">
                                    <label for="" class="col-form-label input-label">Form Type</label>
                                    <select class="form-control" id="form_type" name="form_type">
                                        <option value="single_form">Single Form</option>
                                        <option value="step_form">Step Form</option>
                                    </select>
                                </div>
                                <div class="col-md-12 mt-3">
                                    <label class="col-form-label input-label">Form will be generate for  {{ $record->subServices->parentService->name??'' }} > {{ $record->subServices->name??'' }}</label>
                            ...
                            <div class="text-start mt-3">
                                <input type="hidden" name="is_modify" class="is_modify" value="no">
...
$(document).ready(function() {
    ...
    $(document).on('click', '#save-form', function(event) {
        ...
        $.ajax({
            url:  "{{ baseUrl('/manage-services/generate-assessment-form-save/'.$id) }}",
            type: "post",
            data: formData,
            cache: false,
            contentType: false,
