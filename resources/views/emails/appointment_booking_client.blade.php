		...
		<td mc:edit="text003" class="text_color_282828" style="color: #282828; font-size: 15px; line-height: 2; font-weight: 500; font-family: lato, Helvetica, sans-serif; mso-line-height-rule: exactly;">
			<div class="editable-text" style="line-height: 2;">
				<div class="text_container">
					...
					<p>Your appointment with professional <strong>{{ $professional_name }}</strong> has been successfully <strong>booked and paid</strong> for <strong>{{ $appointment->client_timezone_date ? $appointment->client_timezone_date : 'NA' }}</strong>.</p>

					<p><strong>Appointment Details:</strong></p>
					<ul>
						<li>Porfessional Name: {{ $professional_name }}</li>
						<li>Appointment Date (As per your Timezone): {{ $appointment->client_timezone_date ?? ''  }}</li>
						<li>Appointment Date (As per Professional Timezone): {{ $appointment->appointment_date ? dateFormat($appointment->appointment_date) : 'NA' }}</li>
					...
					<p><a href="{{ baseUrl('/login') }}">Click here to log in</a>.</p>
